...
#	--le output/le.pickle --image {image path}
...
image = fa.align(image, gray, rects[0])
...
box = detections[0, 0, max_i, 3:7] * np.array([w, h, w, h])
(startX, startY, endX, endY) = box.astype("int")
...
if proba > 0.3:
	...
	text = "Unknown"
# text = "{}: {:.2f}%".format(name.capitalize(), proba * 100)
y = startY - 10 if startY - 10 > 10 else startY + 10
cv2.rectangle(image, (startX, startY), (endX, endY),
	(0, 0, 255), 2)
cv2.putText(image, text, (startX, y),
	cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 0, 255), 2)
cv2.imwrite('images/image_recognized.jpg',image)
